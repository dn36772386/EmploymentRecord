...
foreach ($rec in $records) {
  ...
  try {
    ...
    # Lookupフィールドは "フィールド名Id" という形式で設定
    if ($rec.ProjectRef) {
      $values["ProjectCodeLookupId"] = $rec.ProjectRef
    }
...
$items = Get-PnPListItem -List $SrcList -Fields "EmployeeID","FullName","Department","EmploymentType","ProjectCodeLookup" | 
...
foreach ($item in $items) {
  ...
  $project = if($item.FieldValues.ProjectCodeLookup) { $item.FieldValues.ProjectCodeLookup.LookupValue } else { "未設定" }
  Write-Host "  - $empId : $name [$dept / $type] → プロジェクト: $project" -ForegroundColor Gray
}
